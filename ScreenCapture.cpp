...
BOOL CALLBACK MonitorEnumProc(HMONITOR hMonitor, HDC hdcMonitor, LPRECT lprcMonitor, LPARAM dwData)
{
    ...
    {
        HMONITOR* phDefaultMonitor = reinterpret_cast<HMONITOR*>(dwData);
        *phDefaultMonitor = hMonitor;
        return FALSE;
    }
...
{
    HMONITOR hDefaultMonitor = NULL;
    EnumDisplayMonitors(NULL, NULL, MonitorEnumProc, reinterpret_cast<LPARAM>(&hDefaultMonitor));
    return hDefaultMonitor;
}
                     ...
                     _In_ int       nCmdShow)
{
    ...
    OutMgr.InitOutput(g_hMainWnd);

    g_windowCapture = std::make_shared<WindowCapture>(OutMgr.GetD3DDevice());
    g_windowCapture->CaptureMonitor(GetDefaultMonitor());
